...
public class MedicineRequestManager extends Manager<MedicineRequest> {
    private static String csv_path = "data/services/MedicineServices.csv";
    ...
    MedicineRequestManager(Connection connection, String csv_path){
        MedicineRequestManager.connection = connection;
        if(csv_path != null){
            MedicineRequestManager.csv_path = csv_path;
        }
    }
    ...
    void addElement_db(MedicineRequest temp) {
        try {
            ...
            pstmt.setString(4,temp.getTypeMedicine());
            pstmt.setString(5, temp.getCurrentFeeling());
            pstmt.setString(6, temp.getAllergies());
            pstmt.setString(7, temp.getPatientRoom());
            pstmt.executeUpdate();
    ...
    void addElement_map(MedicineRequest temp) {
    ...
    void delElement(String ID) throws SQLException {
        ...
        PreparedStatement pstmt = connection.prepareStatement("DELETE FROM MEDREQ WHERE RequestID=?");
    ...
    @Override
    MedicineRequest getElement(String ID) throws InvalidElementException {
        ...
        if(DatabaseManager.getServiceMap().containsRequest(type,ID)){
            return (MedicineRequest) DatabaseManager.getServiceMap().getRequest(type,ID); //TODO DOES THIS CAST BREAK THINGS
        }else{
