...
public class ExtTransportManager extends Manager<ExternalTransportation> {
    ...
    ExtTransportManager(Connection connection, String csv_path){
        ExtTransportManager.connection = connection;
        if(csv_path != null){
            ExtTransportManager.csv_path = csv_path;
        }
    }
    ...
    public void updateElementMap() throws SQLException {
        ...
        while (result.next()) {
            ExternalTransportation temp = new ExternalTransportation(result.getString(1),result.getString(2),
